...
function getvote($id,$limit,$sn,$sort,$sortby){
  	...
  	$url = 'http://localhost:8888/vote_api_full/voteApi/vote/index.php';
    $param = array();
    if($id != ''){
      $param[0] = 'id='.$id;
    }
...
function submitvote($pb){
  	...
  	$url = 'http://localhost:8888/vote_api_full/voteApi/vote/submit/index.php';
    $post = json_encode($pb);
    $header = array(
      'X-LIMP-SESSION:'.session_id()
    );
...
function alreadyvote($vote_id,$vote_uid){
  	...
  	$url = 'http://localhost:8888/vote_api_full/voteApi/vote/submit/index.php?vid='.$vote_id.'&uid='.$vote_uid;
    $header = array(
      'X-LIMP-SESSION:'.session_id()
    );
  	$result_api = curlget($url,$header);
...
function hashtagInsert($pb){
  $url = 'http://localhost:8888/vote_api_full/voteApi/vote/hashtag/index.php';
  $post = json_encode($pb);
  $header = array(
    'X-LIMP-SESSION:'.session_id()
  );
...
function getVoteTags($vote_id){
  	...
  	$url = 'http://localhost:8888/vote_api_full/voteApi/vote/hashtag/index.php?id='.$vote_id;
    $header = array(
      'X-LIMP-SESSION:'.session_id()
    );
  	$result_api = curlget($url,$header);
